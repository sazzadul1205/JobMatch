// resources/js/pages/Backend/Applications/Create.jsx
...
export default function Create({ job, profile, hasProfile }) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errors, setErrors] = useState({});
  const [fileErrors, setFileErrors] = useState({});

  // Mode: 'profile' or 'custom'
  const [mode, setMode] = useState(hasProfile ? 'profile' : 'custom');

  const [formData, setFormData] = useState({
    name: hasProfile && profile ? `${profile.first_name} ${profile.last_name}` : '',
    email: profile?.email || '',
    phone: profile?.phone || '',
    expected_salary: '',
    resume: null,
  ...
  const [preview, setPreview] = useState({
    ...
    resumeFile: null,
    photo: null,
    showProfileCV: hasProfile && (profile?.cv_path || profile?.cv_url),
    showProfilePhoto: hasProfile && (profile?.photo_path || profile?.photo_url),
  });
  ...
  useEffect(() => {
    if (mode === 'profile' && profile) {
      ...
      setPreview(prev => ({
        ...
        resumeFile: null,
        photo: null,
        showProfileCV: profile.cv_path || profile.cv_url ? true : false,
        showProfilePhoto: profile.photo_path || profile.photo_url ? true : false,
      }));
      // Clear file inputs
  ...
  const removeFile = (type) => {
    if (type === 'resume') {
      ...
      setPreview(prev => ({
        ...
        resumeType: null,
        resumeFile: null,
        showProfileCV: mode === 'profile' && (profile?.cv_path || profile?.cv_url) ? true : false,
      }));
      ...
      setPreview(prev => ({
        ...prev,
        photo: null,
        showProfilePhoto: mode === 'profile' && (profile?.photo_path || profile?.photo_url) ? true : false,
      }));
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === 'custom') {
      // Custom mode - all fields required
      if (!formData.name) newErrors.name = 'Name is required';
      if (!formData.email) newErrors.email = 'Email is required';
      if (!formData.resume) newErrors.resume = 'Resume is required';
    } else {
      // Profile mode - only require resume if profile doesn't have one
      const hasProfileCV = profile?.cv_path || profile?.cv_url;

      // Name and email are auto-filled from profile, so no validation needed
      if (!hasProfileCV && !formData.resume) {
        newErrors.resume = 'No CV found in your profile. Please upload one for this application.';
      }
      // Photo is optional, so no validation needed
    }

    // Check if education criteria is acknowledged
    if (!educationConfirmed && job.education_requirement && job.education_requirement !== 'none') {
    ...
    // Always send mode and data
    if (mode === 'profile') {
      submitData.append('use_profile_data', '1');
      // In profile mode, we only send custom files if they are uploaded
      // The backend will use profile data for missing fields
    } else {
      submitData.append('use_profile_data', '0');
    ...
    router.post(route('applications.store', job.id), submitData, {
      onSuccess: () => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-6 space-y-6">
              {/* Education Requirement Warning Banner */}
              {hasEducationRequirement && (
                <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
                  <div className="flex items-start gap-3">
                    <FaExclamationTriangle className="text-yellow-600 mt-0.5" size={20} />
                    <div>
                      <p className="text-sm font-medium text-yellow-800">Education Requirement</p>
                      <p className="text-sm text-yellow-700 mt-1">
                        This position requires: <strong>{getEducationDisplay(job.education_requirement)}</strong>
                      </p>
                      <p className="text-xs text-yellow-600 mt-2">
                        Please confirm that you meet this requirement. If you are shortlisted for an interview,
                        you will need to provide your educational certificates (original or attested copies).
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-600">
                          {mode === 'profile'
                            ...
                            : 'Enter custom information for this application'}
                        </p>
                      </div>
                    </div>
              ...
              <div>
                ...
                {mode === 'profile' && preview.showProfilePhoto && !preview.photo && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">Profile Photo Available</p>
                        <p className="text-xs text-gray-500">Your saved photo will be used automatically</p>
                      </div>
                    </div>
                ...
                {preview.photo ? (
                ...
                ) : mode === 'profile' && preview.showProfilePhoto ? (
                  <div className="relative inline-block">
                    <img
                      ...
                      className="w-24 h-24 rounded-full object-cover border-2 border-blue-500"
                    />
                    <p className="text-xs text-gray-500 text-center mt-1">Profile Photo</p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      {mode === 'profile' && profile?.photo_path
                        ? 'Upload a different photo for this application (optional)'
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Resume/CV
                  {(mode === 'custom' || (!(profile?.cv_path || profile?.cv_url) && mode === 'profile')) && <span className="text-red-500">*</span>}
                </label>

                {/* Profile CV Indicator */}
                {mode === 'profile' && (profile?.cv_path || profile?.cv_url) && !preview.resume && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <FaFilePdf className="text-green-600" size={20} />
                      <div>
                        <p className="text-sm font-medium text-gray-900">Profile CV Available</p>
                        <p className="text-xs text-gray-500">Your saved CV will be used automatically</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button
                ...
                {preview.resume || mode === 'profile' ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 bg-green-50 text-green-700 px-4 py-3 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">Custom resume for this application</p>
                      </div>
                      <div className="flex gap-2">
                  ...
                  >
                    ...
                    <p className="text-gray-600">
                      {mode === 'profile' && (profile?.cv_path || profile?.cv_url)
                        ? 'Upload a different CV for this application (optional)'
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {mode === 'profile' && (profile?.cv_path || profile?.cv_url)
                    ? 'Leave empty to use your profile CV, or upload a new one for this application'
                    : mode === 'profile' && !(profile?.cv_path || profile?.cv_url)
                      ? 'No CV found in your profile. Please upload one for this application.'
